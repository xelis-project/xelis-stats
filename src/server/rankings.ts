...
const QUERIES: Record<string, (dim: string) => string> = {
  miners: (dim) => `SELECT address, SUM(blocks_found) blocks, SUM(blocks_found) - SUM(sync_count) - SUM(side_count) normal, SUM(sync_count) sync, SUM(side_count) side, SUM(rewards_earned) rewards FROM daily_miners ${dim} GROUP BY address`,
