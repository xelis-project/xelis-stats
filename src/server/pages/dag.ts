...
// Interactive block-DAG page. The shell is server-rendered with the topoheight
// from ?topo= (or the current tip) so a shared link opens at the right window;
// the canvas viewer itself is lazy-loaded client-side (src/client/dag.ts).
dag.get("/dag", async (c) => {
  ...
  const content = `<div class="panel dag-panel" id="dag-app" data-topo="${topo}" data-live="${live ? "1" : "0"}">
    <div class="panel-head dag-head">
      <h2>DAG Viewer${context ? ` <span class="dag-sub">${context}</span>` : ""}</h2>
    ...
    <p class="dag-status" id="dag-status"></p>
  </div>
  <noscript><div class="panel"><h2>DAG Viewer</h2><p style="color:var(--text-dim)">The interactive DAG needs JavaScript. Browse the <a href="/blocks">block list</a> instead.</p></div></noscript>`;

  return c.html(layout("DAG", content, "/dag", "layout-wide"));
});
