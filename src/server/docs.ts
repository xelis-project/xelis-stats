...
const ENDPOINTS: Array<[string, string, string]> = [
  ...
  ["GET", "/api/history/:metric?range=7d|30d|90d|1y|all&interval=day|week|month|year&format=json|csv", "Time-series. Metrics: txs, transfers, accounts, active-accounts, miners, hashrate, difficulty, cum-difficulty, hashprice, fees, fee-p90, supply, burned-supply, market-cap, miner-rev-usd, miner-revenue, side-blocks, block-time, nakamoto, gini, encrypted, block-types, txs-transfer, txs-burn, txs-invoke, txs-deploy, txs-multisig, contract-invokes, contract-gas, contract-deploys, active-contracts, price, quote-volume, mempool, chain-size, peers, peers-hidden, peers-pruned, peers-lagging, peers-stale, peers-divergent, peers-new, peer-lag, peer-view, peer-age, peer-traffic-in, peer-traffic-out"],
